<template>
  <div>
    <div
      class="fixed xl:w-3/4 lg:w-3/4 md:w-3/4 mx-auto rounded-lg position bg-gray-600 z-20 text-white border border-lightMountain"
      :style="contentStyle"
    ...
    >
      <div v-if="contentReceived" class="p-8 pt-6 relative h-full">
        <div class="absolute top-0 right-0 flex mt-2">
          <save-icon class="cursor-pointer"></save-icon>
          <x-circle-icon @click="setContentReceived(false)" class="cursor-pointer mx-4"></x-circle-icon>
        </div>
        <div
        ...
        <div class="bg-darkMountain p-6 rounded overflow-auto" style="max-height: 95%;">
          ...
          >
            ...
            ></arrow-right-circle-icon>
            <img :src="selectedImage" class="mx-auto max-h-full max-w-full" alt="" />
          </div>
          <div v-for="(el, i) in content" :key="i">
            ...
            <div v-else class="text-sm">
              <input type="checkbox" :value="el.text" v-model="stayedArray" />
              {{ el.text }}
            </div>
          </div>
        </div>
...
export default {
  ...
  created () {
    this.images.push({ src: '../../assets/no-image-icon.png', type: 'no-image' })
...
button-shadow {
  -webkit-box-shadow: 0px 0px 25px 31px rgba(255, 255, 255, 1);
  -moz-box-shadow: 0px 0px 25px 31px rgba(255, 255, 255, 1);
  box-shadow: 0px 0px 25px 31px rgba(255, 255, 255, 1);
}
</style>
